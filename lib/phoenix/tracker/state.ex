defmodule Phoenix.Tracker.State do
  ...
  def merge(local, remote) do
    :telemetry.span(
      [:phoenix_pubsub, :merge_state],
      %{},
      fn -> {merge0(local, remote), %{}} end
    )
  end

  @spec merge0(local :: t, {remote :: t, values} | delta) ::
          {new_local :: t, joins :: [value], leaves :: [value]}
  defp merge0(%State{} = local, %State{mode: :delta, values: remote_map} = remote) do
    merge0(local, remote, remote_map)
  end

  defp merge0(%State{} = local, {%State{} = remote, remote_map}) do
    merge0(local, remote, remote_map)
  end

  defp merge0(local, remote, remote_map) do
  ...
  def merge_deltas(local, remote) do
    :telemetry.span(
      [:phoenix_pubsub, :merge_deltas],
      %{},
      fn -> {merge_deltas0(local, remote), %{}} end
    )
  end
